...
jq -c '.[]' ./data/accounts.json | while read i; do
  name=$(jq -r '.name' <<< "$i")
  pub=$(jq -r '.publicKey' <<< "$i")
  pri=$(jq -r '.privateKey' <<< "$i")

  cleos wallet create -n $name --to-console | tail -1 | sed -e 's/^"//' -e 's/"$//' > /opt/eosio/bin/"$name"_wallet_password.txt
  cleos wallet import -n $name  --private-key $pri
